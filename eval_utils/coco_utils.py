...
def get_coco_category_maps(annotations_file):
    ...
    参数:
        annotations_file (str): 注释文件的路径

    返回:
        1) cats_to_classes: 字典   key:原始 value:变换后的ID  original (keys) and the transformed category IDs (values).
        2) classes_to_cats: 字典    the transformed (keys) and the original category IDs (values).
        3) cats_to_names: 字典      original category IDs (keys) and the respective category names (values).
        4) classes_to_names: 列表   A list of the category names (values) with their indices representing the transformed IDs.
    '''
