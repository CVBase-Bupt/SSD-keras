...
def greedy_nms(y_pred_decoded, iou_threshold=0.45, coords='corners', border_pixels='half'):
    ...
    for batch_item in y_pred_decoded: # For the labels of each batch item...
        boxes_left = np.copy(batch_item) #留下来的boxes
        maxima = [] # 存储nms留下来的box
        while boxes_left.shape[0] > 0: # 仍然有需要比较的boxes
            maximum_index = np.argmax(boxes_left[:,1]) # ...得到下一个分数最高的box的索引
            maximum_box = np.copy(boxes_left[maximum_index]) # ...copy that box and...
            maxima.append(maximum_box) # 将最大的box存储至maximum_box
            boxes_left = np.delete(boxes_left, maximum_index, axis=0) # 从 `boxes_left`中移除最大的box
            if boxes_left.shape[0] == 0: break # 如果boxes_left中为空，停止循环.
            similarities = iou(boxes_left[:,2:], maximum_box[2:], coords=coords, mode='element-wise', border_pixels=border_pixels) # .计算剩余boxes和最大box的iou
            boxes_left = boxes_left[similarities <= iou_threshold] # 移除和最大box的iou超出阈值的box
...
def _greedy_nms(predictions, iou_threshold=0.45, coords='corners', border_pixels='half'):
    '''
    per-class NMS in `decode_detections()`.
    '''
...
def decode_detections_fast(y_pred,
    ...
    # 1: 类别 one-hot encoding -> class ID
    ...
    # 2: predicted anchor box offsets -> absolute coordinates
    if input_coords == 'centroids':
    ...
    # 3: normalized box coordinates -> absolute coordinates
    if normalize_coords:
