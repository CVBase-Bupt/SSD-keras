...
class SSDExpand:
    '''    
    zoom in： random crops 
    zoom out： 在random crops之前使用均值插值方式将图像放大16x
    '''
...
class SSDPhotometricDistortions:
    ...
    def __init__(self):
        ...
        self.random_brightness = RandomBrightness(lower=-32, upper=32, prob=0.5) #亮度
        self.random_contrast = RandomContrast(lower=0.5, upper=1.5, prob=0.5)    #对比度
        self.random_saturation = RandomSaturation(lower=0.5, upper=1.5, prob=0.5)#饱和度
        self.random_hue = RandomHue(max_delta=18, prob=0.5) #色调
