...
class AnchorBoxes(Layer):
    '''
    一个Keras层，根据输入的张量和传递的参数输出default box坐标和variances 

    输入 shape:
       4维 `(batch, height, width, channels)` 如果 `dim_ordering = 'tf'`.
       
    输出 shape:
        5维  `(batch, height, width, n_boxes, 8)`. 
        8包括default box的4个坐标，以及每个box的四个variance值。
    '''
    ...
    def call(self, x, mask=None):
        ...
        # 计算偏移值, i.e. at what pixel values the first anchor box center point will be from the top and from the left of the image.
        ...
        # 将 `(cx, cy, w, h)` 格式转成 `(xmin, xmax, ymin, ymax)`
